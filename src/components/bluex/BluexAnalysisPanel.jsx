  ...
  if (!analysis) {
    return (
      <div className="text-center py-12 bg-gray-50 rounded-2xl border border-dashed border-gray-200">
        ...
        <p className="text-sm font-semibold text-gray-700">Sin análisis aún</p>
        <p className="text-xs text-gray-500 mt-1">Click en "Analizar con IA" para que el agente revise tu operación logística.</p>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-4">
      ...
      {ai.patrones?.length > 0 && (
        <div className="bg-white border border-gray-200 rounded-2xl p-5">
          <h3 className="font-bold text-gray-800 flex items-center gap-2 mb-3 text-sm">
            <TrendingUp className="w-4 h-4 text-cyan-600" /> Patrones detectados
          </h3>
          <ul className="space-y-2">
            {ai.patrones.map((p, i) => (
              <li key={i} className="flex items-start gap-2 text-sm text-gray-700">
                <span className="text-cyan-500 mt-0.5 font-bold">•</span><span>{p}</span>
              </li>
            ))}
          </ul>
        </div>
      ...
      {ai.sugerencias?.length > 0 && (
        <div className="bg-white border border-gray-200 rounded-2xl p-5">
          <h3 className="font-bold text-gray-800 flex items-center gap-2 mb-3 text-sm">
            <Award className="w-4 h-4 text-emerald-600" /> Acciones recomendadas
          </h3>
            ...
            {ai.sugerencias.map((s, i) => (
              <div key={i} className={`border rounded-xl p-3 ${PRIORIDAD_STYLES[s.prioridad] || PRIORIDAD_STYLES.Media}`}>
                <div className="flex items-start justify-between gap-2 mb-1">
                  ...
                  <span className="text-[9px] uppercase tracking-wider font-bold px-1.5 py-0.5 rounded-full bg-white/70">{s.prioridad}</span>
                </div>
                <p className="text-xs leading-relaxed">{s.descripcion}</p>
              </div>
            ))}
          </div>
        </div>
      ...
      {analysis.comunas_top_problemas?.length > 0 && (
        <div className="bg-white border border-gray-200 rounded-2xl p-5">
          <h3 className="font-bold text-gray-800 flex items-center gap-2 mb-3 text-sm">
            <AlertTriangle className="w-4 h-4 text-amber-600" /> Comunas con más incidencias
          </h3>
            ...
            {analysis.comunas_top_problemas.slice(0, 8).map((c, i) => (
              <div key={i} className="flex items-center justify-between py-2 border-b border-gray-100 last:border-0">
                <div className="flex items-center gap-2">
                  ...
                  <span className="font-semibold text-gray-800">{c.comuna}</span>
                </div>
                <div className="flex items-center gap-3 text-[11px]">
                  <span className="text-gray-500">{c.total} envíos</span>
...
function Stat({ label, val, accent = 'slate' }) {
  const colors = { slate: 'text-gray-800', emerald: 'text-emerald-600', red: 'text-red-600' };
  return (
    <div className="text-center">
      <p className="text-[9px] uppercase tracking-wider font-bold text-gray-500">{label}</p>
      <p className={`font-extrabold text-lg tabular-nums ${colors[accent]}`}>{val ?? '—'}</p>
    </div>
  );
}