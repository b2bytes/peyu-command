...
// Diseño: paleta azul marino "Día del Padre", glass premium, mobile-first.
...
const ACCENT = '#0EA5E9';   // azul cielo
const NAVY = '#0C4A6E';     // azul marino
...
export default function FathersDayQuoteModal({ open, onClose }) {
  ...
  return (
    <div className="fixed inset-0 z-[200] flex items-end sm:items-center justify-center" role="dialog" aria-modal="true">
      ...
      {/* Modal */}
      <div
        className="relative w-full sm:max-w-lg max-h-[92vh] flex flex-col rounded-t-[2rem] sm:rounded-[2rem] overflow-hidden shadow-2xl animate-in slide-in-from-bottom sm:zoom-in-95 duration-300"
        style={{ background: `linear-gradient(160deg, ${NAVY} 0%, #0B1220 60%, #060A14 100%)` }}
      >
        {/* Glow decorativo */}
        <div aria-hidden className="pointer-events-none absolute -top-20 -right-16 w-64 h-64 rounded-full blur-3xl opacity-30" style={{ background: ACCENT }} />

        {/* Header */}
        <div className="relative flex items-center justify-between px-5 pt-5 pb-3 flex-shrink-0">
          <div className="flex items-center gap-2.5 min-w-0">
            <div className="w-10 h-10 rounded-2xl flex items-center justify-center text-xl shadow-lg flex-shrink-0" style={{ background: `linear-gradient(135deg, ${ACCENT}, ${ACCENT}cc)` }}>
              👔
            </div>
            <div className="min-w-0">
              <p className="text-white font-poppins font-extrabold text-base leading-none truncate">Regalo Día del Padre</p>
              <p className="text-[11px] text-sky-200/80 mt-1 leading-none">Domingo 21 de junio · Chile 🇨🇱</p>
            </div>
          </div>
          <button onClick={onClose} className="w-9 h-9 rounded-xl bg-white/10 hover:bg-white/20 flex items-center justify-center text-white transition flex-shrink-0" aria-label="Cerrar">
            <X className="w-4 h-4" />
          </button>
        </div>
        ...
        <div className="relative flex items-center gap-2 px-5 pb-3 flex-shrink-0">
          ...
          <span className="text-[10px] font-bold text-sky-200/80 ml-1 tabular-nums">Paso {step + 1}/2</span>
        </div>

        {/* Body */}
        <div className="relative flex-1 overflow-y-auto px-5 pb-5 peyu-scrollbar-light">
          {/* ─── PASO 0 — Segmentación ─── */}
          {step === 0 && (
            <div className="space-y-3 animate-in fade-in duration-200">
              ...
              <p className="text-sky-100/70 text-sm leading-snug">
              ...
              <button
                ...
                className="w-full text-left p-4 rounded-2xl border-2 border-white/15 bg-white/[0.05] hover:border-sky-400 hover:bg-white/[0.09] transition-all group"
              >
                <div className="flex items-start gap-3">
                  ...
                  <div className="min-w-0 flex-1">
                    <p className="text-white font-bold text-[15px] flex items-center gap-1.5">
                      ...
                      <ArrowRight className="w-4 h-4 text-sky-300 opacity-0 group-hover:opacity-100 group-hover:translate-x-0.5 transition-all" />
                    </p>
                    <p className="text-sky-100/65 text-xs mt-1 leading-snug">
              ...
              >
                <div className="flex items-start gap-3">
                  <div className="w-12 h-12 rounded-2xl bg-gradient-to-br from-slate-300 to-slate-500 flex items-center justify-center flex-shrink-0 shadow-lg">
              ...
              <div className="flex items-center gap-1.5 text-[11px] text-sky-200/60 justify-center pt-1">
          ...
          {step === 1 && segmento === 'persona' && (
            <div className="space-y-3 animate-in fade-in slide-in-from-right-2 duration-200">
              <h3 className="text-white font-poppins font-bold text-lg leading-tight">Elige su regalo 🎁</h3>
              <p className="text-sky-100/70 text-sm leading-snug">Productos favoritos para papá. Toca para agregar y ajusta la cantidad.</p>

              {loadingProds ? (
                <div className="grid grid-cols-2 gap-2.5">
                  {[1, 2, 3, 4].map(i => (
                    <div key={i} className="rounded-2xl bg-white/[0.04] border border-white/10 aspect-[4/5] animate-pulse" />
                  ))}
                </div>
              ) : (
                <div className="grid grid-cols-2 gap-2.5">
                  {productos.map(p => {
                    ...
                    return (
                      <div
                        key={p.id}
                        className={`rounded-2xl overflow-hidden border-2 transition-all ${active ? 'border-sky-400 bg-sky-500/10' : 'border-white/10 bg-white/[0.04]'}`}
                      >
                        <button onClick={() => toggleProd(p)} className="block w-full relative">
                          ...
                          {active && (
                            <div className="absolute top-1.5 right-1.5 w-5 h-5 rounded-full bg-sky-400 flex items-center justify-center shadow">
                              <CheckCircle className="w-3.5 h-3.5 text-slate-900" strokeWidth={3} />
                            </div>
                          )}
                        </button>
                        <div className="p-2">
                          <p className="text-white text-[11px] font-semibold leading-tight line-clamp-2 min-h-[28px]">{p.nombre}</p>
                          <p className="text-sky-300 text-xs font-bold mt-0.5">{clp(p.precio_b2c)}</p>
                          {active && (
                            <div className="flex items-center justify-between mt-1.5 bg-white/10 rounded-lg p-0.5">
                              <button onClick={() => setQtyProd(p, -1)} className="w-6 h-6 rounded-md bg-white/10 hover:bg-white/20 flex items-center justify-center text-white"><Minus className="w-3 h-3" /></button>
                              <span className="text-white text-xs font-bold tabular-nums">{qty}</span>
                              <button onClick={() => setQtyProd(p, 1)} className="w-6 h-6 rounded-md bg-white/10 hover:bg-white/20 flex items-center justify-center text-white"><Plus className="w-3 h-3" /></button>
                            </div>
                          )}
                        </div>
                      </div>
          ...
          {step === 1 && segmento === 'empresa' && (
            <div className="space-y-3 animate-in fade-in slide-in-from-right-2 duration-200">
              <h3 className="text-white font-poppins font-bold text-lg leading-tight">Cotización corporativa</h3>
              <p className="text-sky-100/70 text-sm leading-snug">Déjanos tus datos y armamos tu propuesta con logo y precios por volumen al instante.</p>
              ...
              <div className="space-y-2.5">
                ...
                <div>
                  <label className="text-[10px] font-bold text-sky-200/70 uppercase tracking-wider mb-1.5 block">¿Cuántos papás en tu equipo?</label>
                  <div className="flex items-center gap-2">
                    {[10, 20, 50, 100].map(n => (
                      <button
                        key={n}
                        onClick={() => setForm({ ...form, qty: n })}
                        className={`flex-1 py-2.5 rounded-xl text-sm font-bold border transition-all ${form.qty === n ? 'border-sky-400 bg-sky-500/20 text-white' : 'border-white/15 bg-white/[0.04] text-sky-100/70 hover:bg-white/10'}`}
                      >
                        {n}+
                      </button>
                    ))}
              ...
              <div className="bg-white/[0.05] border border-white/10 rounded-2xl p-3 space-y-1.5 mt-1">
                {[
                  ...
                  <div key={i} className="flex items-center gap-2 text-sky-100/85 text-xs">
                    <b.icon className="w-3.5 h-3.5 text-sky-300 flex-shrink-0" />
                    <span>{b.t}</span>
                  </div>
                ))}
              </div>
        ...
        {/* Footer / CTA */}
        {step === 1 && (
          <div className="relative flex items-center gap-2 px-5 py-4 border-t border-white/10 flex-shrink-0 bg-black/20" style={{ paddingBottom: 'calc(env(safe-area-inset-bottom) + 1rem)' }}>
            <button
              onClick={() => { setStep(0); setSegmento(null); }}
              className="w-11 h-12 rounded-xl bg-white/10 hover:bg-white/20 flex items-center justify-center text-white transition flex-shrink-0"
              aria-label="Atrás"
            >
            ...
            {segmento === 'persona' ? (
              <button
                ...
                className="flex-1 h-12 rounded-xl font-bold text-white flex items-center justify-center gap-2 transition-all disabled:opacity-40 shadow-lg"
                style={{ background: itemsPersona ? `linear-gradient(135deg, ${ACCENT}, #2563EB)` : '#334155' }}
              >
              ...
              <button
                ...
                className="flex-1 h-12 rounded-xl font-bold text-white flex items-center justify-center gap-2 transition-all disabled:opacity-40 shadow-lg"
                style={{ background: empresaValida ? `linear-gradient(135deg, ${ACCENT}, #2563EB)` : '#334155' }}
              >
...
function Chip({ icon: Icon, children }) {
  return (
    <span className="inline-flex items-center gap-1 text-[10px] font-semibold text-sky-100/80 bg-white/10 border border-white/10 rounded-full px-2 py-0.5">
...
function Field({ label, value, onChange, placeholder, type = 'text' }) {
  return (
    <div>
      <label className="text-[10px] font-bold text-sky-200/70 uppercase tracking-wider mb-1.5 block">{label}</label>
      <Input
        type={type}
        value={value}
        onChange={e => onChange(e.target.value)}
        placeholder={placeholder}
        className="h-11 rounded-xl bg-white/[0.06] border-white/15 text-white placeholder:text-white/30 focus-visible:border-sky-400/60 focus-visible:bg-white/[0.10]"
      />
    </div>
  );
}