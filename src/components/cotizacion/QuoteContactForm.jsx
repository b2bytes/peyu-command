...
const Field = ({ icon: Icon, ...props }) => (
  <div className="relative">
    <Icon className="absolute left-3 top-1/2 -translate-y-1/2 w-3.5 sm:w-4 h-3.5 sm:h-4 text-[#A78B6F]" />
    <input
      {...props}
      className="w-full h-10 sm:h-11 pl-9 sm:pl-10 pr-3 sm:pr-4 rounded-xl bg-white border border-[#EBE3D6] text-xs sm:text-sm text-[#2A2420] placeholder:text-[#A78B6F] focus:outline-none focus:border-[#0F8B6C] focus:ring-2 focus:ring-[#0F8B6C]/15"
    />
  </div>
);
...
const Section = ({ icon: Icon, title, hint, children }) => (
  <div>
    <div className="flex items-center gap-2 sm:gap-2.5 mb-3 sm:mb-3.5">
      <span className="w-6 sm:w-7 h-6 sm:h-7 rounded-lg bg-[#0F8B6C]/8 flex items-center justify-center flex-shrink-0">
        <Icon className="w-3 sm:w-3.5 h-3 sm:h-3.5 text-[#0F8B6C]" />
      </span>
      <div>
        <p className="text-xs sm:text-sm font-bold text-[#2A2420] leading-tight">{title}</p>
        {hint && <p className="text-[10px] sm:text-[11px] text-[#A78B6F] leading-tight">{hint}</p>}
      </div>
    </div>
  ...
  return (
    <div className="space-y-6">
      ...
      <Section icon={MessageSquare} title="Sobre tu pedido">
        {/* Urgencia */}
        <div>
          <p className="text-[10px] sm:text-xs font-semibold text-[#4B4F54] mb-2 sm:mb-2.5 flex items-center gap-1.5">
            <Zap className="w-3 sm:w-3.5 h-3 sm:h-3.5 text-[#D96B4D]" /> ¿Qué tan urgente es?
          </p>
          <div className="grid grid-cols-3 gap-2 sm:gap-2.5">
            {URGENCIAS.map((u) => (
              <button
                ...
                className={`h-9 sm:h-11 rounded-lg sm:rounded-xl border text-[10px] sm:text-sm font-bold transition-all ${
                  form.urgency === u.v
                    ? 'bg-[#0F8B6C] border-[#0F8B6C] text-white'
                    : 'bg-white border-[#EBE3D6] text-[#4B4F54] hover:border-[#0F8B6C]/40'
                }`}
              >
                <span className="mr-0.5 sm:mr-1">{u.emoji}</span><span className="hidden sm:inline">{u.label}</span><span className="sm:hidden">{u.label.slice(0, 2)}</span>
              </button>
            ))}
          </div>
        </div>

        <label className="flex items-center gap-2 sm:gap-2.5 bg-white border border-[#EBE3D6] rounded-lg sm:rounded-xl px-3 sm:px-3.5 py-2.5 sm:py-3 cursor-pointer">
          <input
            type="checkbox"
            checked={form.personalization_needs}
            onChange={(e) => setForm({ ...form, personalization_needs: e.target.checked })}
            className="w-3.5 sm:w-4 h-3.5 sm:h-4 accent-[#0F8B6C] flex-shrink-0"
          />
          <Sparkles className="w-3.5 sm:w-4 h-3.5 sm:h-4 text-[#D96B4D] flex-shrink-0" />
          <span className="text-xs sm:text-sm font-semibold text-[#2A2420]">Personalización con logo (láser)</span>
        </label>

        <div className="relative">
          <FileText className="absolute left-3 top-2.5 sm:top-3.5 w-3.5 sm:w-4 h-3.5 sm:h-4 text-[#A78B6F]" />
          <textarea
            rows={3}
            placeholder="¿Algo más que debamos saber? (uso, evento, colores, plazos...)"
            value={form.notes}
            onChange={set('notes')}
            className="w-full pl-9 sm:pl-10 pr-3 sm:pr-4 py-2.5 sm:py-3 rounded-lg sm:rounded-xl bg-white border border-[#EBE3D6] text-xs sm:text-sm text-[#2A2420] placeholder:text-[#A78B6F] focus:outline-none focus:border-[#0F8B6C] focus:ring-2 focus:ring-[#0F8B6C]/15 resize-none"
          />
        </div>
      </Section>
    </div>
