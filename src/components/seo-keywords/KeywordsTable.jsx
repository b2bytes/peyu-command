      ...
      <div className="px-3 md:px-4 py-3 border-b border-slate-800 space-y-3">
        ...
        <div className="relative">
          ...
          <input
            ...
            className="w-full bg-slate-950 border border-slate-800 rounded-lg pl-9 pr-9 py-2.5 text-[14px] text-slate-50 placeholder:text-slate-500 font-inter focus:outline-none focus:border-teal-500/50 focus:bg-slate-950"
          />
          {search && (
            <button
              onClick={() => setSearch('')}
              className="absolute right-2 top-1/2 -translate-y-1/2 w-6 h-6 rounded-md flex items-center justify-center text-slate-300 hover:text-slate-50 hover:bg-slate-800"
            >
          ...
          {SORT_OPTIONS.map(s => (
            <button
              ...
              className={`px-2 py-0.5 rounded text-[11px] font-bold transition-colors whitespace-nowrap ${
                sortBy === s.k ? 'text-teal-300' : 'text-slate-400 hover:text-slate-50'
              }`}
            >
              {s.l}
            </button>
            ...
            <div className="flex items-center justify-between text-[11px] font-mono pl-6">
              <div className="flex items-center gap-3">
                ...
                <span className="text-slate-300">
                  ...
                  <span className="text-slate-50 font-bold">{(k.ctr * 100).toFixed(1)}%</span>
                </span>
              </div>
              <a
                ...
                className="inline-flex items-center justify-center w-7 h-7 rounded-md bg-slate-800 hover:bg-teal-500/20 hover:text-teal-300 text-slate-300 transition-colors flex-shrink-0"
          ...
          <tbody className="divide-y divide-slate-800">
            {filtered.map((k, i) => (
              <tr key={k.query} className="hover:bg-slate-800/40 transition-colors">
                <td className="px-4 py-2.5 text-slate-500 font-mono text-[11px]">{i + 1}</td>
                <td className="px-4 py-2.5 text-slate-50 font-medium font-inter">{k.query}</td>
                <td className="px-4 py-2.5 text-right font-jakarta font-bold text-slate-50 font-mono">
                  {k.position.toFixed(1)}
                </td>
                <td className="px-4 py-2.5 text-right text-slate-200 font-mono">{k.impressions}</td>
                <td className="px-4 py-2.5 text-right text-slate-200 font-mono">{k.clicks}</td>
                <td className="px-4 py-2.5 text-right text-slate-300 font-mono text-[12px]">
                  {(k.ctr * 100).toFixed(1)}%
                </td>
                ...
                <td className="px-4 py-2.5 text-center">
                  <a
                    ...
                    target="_blank"
                    rel="noopener noreferrer"
                    className="inline-flex items-center justify-center w-7 h-7 rounded-md bg-slate-800 hover:bg-teal-500/20 hover:text-teal-300 text-slate-300 transition-colors"
                    title="Probar en Google.cl"
                  >
                    <ExternalLink className="w-3.5 h-3.5" />
                  </a>
