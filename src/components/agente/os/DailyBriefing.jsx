// ============================================================================
// PEYU OS · Briefing del día — Command Cockpit (sage oscuro)
// ─────────────────────────────────────────────────────────────────────────────
...
import { Loader2, ShoppingBag, Target, MessageCircle, Truck, AlertTriangle } from 'lucide-react';
...
function StatTile({ icon: Icon, label, value, sub, accent, onClick }) {
  return (
    <button
      onClick={onClick}
      className="text-left rounded-2xl bg-[#10231d] border border-[#2a4a40] p-3.5 hover:border-[#0F8B6C]/50 hover:bg-[#12281f] transition group"
    >
      <div className="flex items-center gap-2 mb-1.5">
        <span className={`w-7 h-7 rounded-xl flex items-center justify-center ${accent}`}>
          <Icon className="w-3.5 h-3.5" />
        </span>
        <span className="text-[11px] font-medium text-[#7fa295] uppercase tracking-wide">{label}</span>
      </div>
      <p className="text-xl font-poppins font-bold text-[#eaf5f0] leading-none">{value}</p>
      {sub && <p className="text-[11px] text-[#6f8f83] mt-1">{sub}</p>}
    </button>
  );
}
...
export default function DailyBriefing({ onAsk }) {
  ...
  if (loading) {
    return (
      <div className="flex items-center justify-center gap-2 text-[#6f8f83] py-6">
    ...
    <div className="w-full max-w-lg mx-auto mt-6 space-y-3">
      ...
      {alertas.length > 0 && (
        <button
          ...
          className="w-full text-left rounded-2xl bg-[#2a1a14] border border-[#5a3527] p-3.5 hover:border-[#D96B4D]/60 transition"
        >
          <div className="flex items-center gap-2 mb-1.5">
            <AlertTriangle className="w-4 h-4 text-[#f0a085]" />
            <span className="text-xs font-semibold text-[#f0a085] uppercase tracking-wide">Requiere atención</span>
          </div>
          <p className="text-sm text-[#e8c4b6] leading-relaxed">{alertas.join(' · ')}</p>
        </button>
      )}
    </div>
  );
